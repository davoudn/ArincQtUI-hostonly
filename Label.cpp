...
int Label::initFromDatabase(str_t path, str_t equipment_id, str_t label_id)
{
    ...
    this->equipment_id  = label_data->equipmentId;
    ...
    for (const auto& param : params_list) {
        ...
        if (param.type == "BCD") {
            ...
            base_param = new Param<DArincParamType::DISCRETE>(param.name, param.details);
...
BaseItem* Label::getParameter(str_t pname) const
{
...
{
    str_t tmp;
    for (uint32_t i=sdiLsb; i<=arinc_data.size();i++)
    {
...
DArincData& Label::getArincData() const
{
        return arinc_data;
}
