...
bool DEI1016::sendEvent(BaseEvent* ac)
{
    if (ac){
...
{
        int numSent = serial->write(ac, POCKET_SIZE);
        if (numSent==POCKET_SIZE){
            qInfo() << "Num bytes sent: "<<numSent;
            return true;
        }
        else {
...
void DEI1016::dataReceived()
{
    ...
    while (data.size() >= POCKET_SIZE)
    {
        char packet[POCKET_SIZE];
        memcpy(packet, data.data(), POCKET_SIZE);
        data.remove(0, POCKET_SIZE);
        ...
        AUX::convertBytesToData(packet, dei, chanell, rate, arincData);
...
void DEI1016::updateTask(char* data)
{
    uint8_t  chanell = 0;
    uint8_t  dei = 0;
    float    rate = 0.f;
    ...
    AUX::convertBytesToData(data, dei, chanell, rate, arincData);
    AUX::convertFromDEIToArinc(arincData);
    ReceiverWorker::getInstance(0)->update(dei, chanell, rate, arincData);
}
...
void DEI1016::openSerialPort()
{
    const SettingsDialog::Settings p = SettingsDialog::getInstance()->settings();
     ...
     serial->setReadBufferSize(POCKET_SIZE);
