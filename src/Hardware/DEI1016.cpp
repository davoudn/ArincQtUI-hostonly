...
void DEI1016::parse(uint8_t byte)
{
    ...
    if ( state==State::FinalReceived)
    {
        ...
        chanel_index = getChanellIndex(receive_data);
        auto r =  receive_data_buffer[chanel_index]
                            .push_back(receive_data);
    }
}
...
/*
 Defining a global  datasource and to assign a 
 callback from igpio library.
*/
...
SpscRingBuffer<record_t, 64>& DEI1016::
getReceiveDataBuffer(uint32_t chanell) 
{
    return receive_data_buffer[chanell];
}
