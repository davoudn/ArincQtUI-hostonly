            ...
            switch (baseItem->type)
            {
                case BaseItem::ItemType::Parameter:
                {
                    ...
                    switch(column) {
                        ...
                        case COLUMN_INFO: retVal =info; break;
                        case COLUMN_ENGVALUE: retVal = label->getValueEng(paramname); break;
                        default: retVal = ""; break;
                    }
                    break;
                }
                case BaseItem::ItemType::Label:
                {
                    ...
                    switch(column) {
                        ...
                        case COLUMN_BINARYVALUE:  retVal = label->getValueBits(); break;
...
void MyDataModel::addLabelAction(uint32_t dei, uint32_t channel, uint32_t transrec, uint32_t instr, Label* label)
{
    ...
    GeneralData::getInstance()->getEvents().push_back(MakeDataEvent(dei, channel, transrec, instr, label->getArincData().getBitSet().to_ulong(), label->getDataRate().toFloat()));
}
