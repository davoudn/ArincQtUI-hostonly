...
value_t DArincData::Get<DataHex>() const {
    int n = ArincFunctions::INT::Value<uint32_t>(9, 32, *this);
    return value_t(str_t::number(n,16));
}

template <>
value_t  DArincData::Get<DataBits>() const {
    return ArincFunctions::BITS::Value<value_t>(LABEL_SIZE+1, ARINC32_SIZE, *this);
}

template <>
void DArincData::Set<DataBits>(const value_t& value){
    if ( value.toString().size() == ARINC32_SIZE-LABEL_SIZE){
        ArincFunctions::BITS::SetValue<value_t>(value, LABEL_SIZE+1, ARINC32_SIZE, *this);
    }
}
...
/*
template <>
void DArincData::Set<SDIBits>(const value_t& value)  {
    ArincFunctions::BITS::SetValue<value_t>(value,9,*this);
}

template <>
void DArincData::Set<SSMBits>(const value_t& value)  {
    ArincFunctions::BITS::SetValue<value_t>(value, 30, *this);
}
...
template <>
value_t DArincData::Get<SSMBits>() const {
    return ArincFunctions::BITS::Value<value_t>(31, 32, *this);
}

template <>
value_t DArincData::Get<SSMInt>() const {
    return ArincFunctions::INT::Value<value_t>(31, 32, *this);
}

template <>
value_t DArincData::Get<SDIInt>() const {
    return ArincFunctions::INT::Value<value_t>(9, 10, *this);
}

template <>
value_t DArincData::Get<SDIBits>()  const {
    return ArincFunctions::BITS::Value<value_t>(9, 10, *this);
}
 * */
