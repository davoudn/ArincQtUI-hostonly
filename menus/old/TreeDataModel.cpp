...
bool MyDataModel::setLabelData(str_t labelId, const float& rate,  const QVariant &value){
    ...
    if (_index.isValid()){
        setData(_index,value, Qt::EditRole);
        QModelIndex __index = index(_index.row(), COLUMN_DATARATE, _index.parent());
...
std::vector<str_t> MyDataModel::getTimeOutList(double _timeOut){
    ...
    if (topindex.isValid()){
        for (int row=0; row < rowCount(topindex); row++){
            ...
            if (_base->type == BaseItem::ItemType::Label){
            LabelFor<DReceiver>* _tmp = static_cast<LabelFor<DReceiver>*>(_base);
            if (_tmp){
                    if ( _tmp->timeoutCheckAndIncrement(_timeOut) ){
...
void  MyDataModel::cleanTimeoutList(double _timeOut){
    auto _garbage = getTimeOutList(_timeOut);
...
std::vector<DArincData> MyDataModel::getListOfAvailableLabelData(){
    std::vector<DArincData> _listOfDataToSend;
    QModelIndex topindex = index(0,0,QModelIndex());
    if (topindex.isValid()){
        for (int row=0; row < rowCount(topindex); row++){
            QModelIndex _index = index(row,0,topindex);
            BaseItem* _base = static_cast<BaseItem*>(_index.internalPointer());
            if (_base->type == BaseItem::ItemType::Label){
            LabelFor<DTransmitter>* _tmp = static_cast<LabelFor<DTransmitter>*>(_base);
            if (_tmp){
                    if ( _tmp->getIfDataAvailable() ){
                        _listOfDataToSend.push_back(_tmp->getArincDataAndReset());
                    }
                }
            }
        }
