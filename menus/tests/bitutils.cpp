...
void convertDataToBytes(uint32_t& chanel, uint32_t& trancive, uint32_t& instruction, float& rate, uint32_t arincData, char* recData){

    recData[Byte0] = chanel;
    recData[Byte1] = timeToBits(rate);
    recData[INSTRUCTION_BYTE] = makeInstructionByte(chanel, trancive, instruction);
    recData[RATE_BYTE] = AUX::timeToBits(rate);
    std::bitset<ARINC32_SIZE> a(arincData);
    AUX::convertFromArincToDEI(a);
    uint32_t dword = static_cast<uint32_t>(a.to_ulong());
    AUX::split(dword, recData[Byte4],recData[Byte5],recData[Byte6],recData[Byte7]);
}
